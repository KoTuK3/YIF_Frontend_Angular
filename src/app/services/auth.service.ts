...
export class AuthService {
  ...
  public login(user: any) {
    let loginUrl: string = BASE_URL + "/Authentication/LoginUser";
    return this.http.post(loginUrl, user).pipe(
      tap(this.setTokens));
  }

  public register(user: any) {
    let registerUrl: string = BASE_URL + "/Authentication/RegisterUser";
    return this.http.post(registerUrl, user).pipe(
      tap(this.setTokens));
  }

  private async refreshTokens(model: any) {
    let refreshTokenUrl: string = BASE_URL + "/Authentication/RefreshToken";
